...
// @TODO: retry logic
var TIME_TO_RETR_MISSING = 6 * 1000

var setupIPFS = thunky(function(cb) {
	var node = new IPFS({ 
		EXPERIMENTAL: {
		 	pubsub: true
		},
	...
	// @TODO
	node.on('error', function(err) { console.error(err) })
})

function ipfsTransport(url)
{
	var manifestUrl = url.replace('ipfs://', '/ipfs/').replace('ipns://', '/ipns/')
	var base = manifestUrl.replace('/manifest.json', '/')
	...
	this.manifest = function(cb)
	{
		setupIPFS(function(err, node) {
			...
			retrFile(node, manifestUrl, function(err, resp) {
				...
				//node1.peerRouting.findPeer(node3.peerInfo.id, function(err, peer) {
...
ipfsTransport.isValidURL = function(url) {
	var parsed = URL.parse(url)
	return parsed.protocol === 'ipfs:' || parsed.protocol === 'ipns:'
}
module.exports = ipfsTransport
