...
module.exports = function legacyTransport(url)
{
	...
	this.get = function(args, cb) 
	{
		...
		if (resource == 'catalog') {
			if (extra && extra.search) {
				jsonRPCRequest('meta.search', [null, remapSearch(args)], wrapSearchResp(args, cb))
			} else {
	...
	function remapSearch(args)
	{
		let extra = args[3]
		return { query: extra.search }
	}

	function wrapSearchResp(args, cb) 
	{
		return function(err, res) {
			if (err) return cb(err)
			return cb(null, { metas: res.results, query: args[3].search })
		}
	}
